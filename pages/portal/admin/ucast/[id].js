...
export default function EventAttendance() {
  ...
  return <RequireAuth><PortalHeader />
    <main className="max-w-6xl mx-auto px-4 py-6">
      ...
      <h1 className="text-2xl font-bold mt-4">Přihlášení účastníci</h1>
      {data && <h2 className="text-lg mt-2 font-semibold">{data.event.title}</h2>}
      <p className="text-slate-600 mt-2">Seznam osob, které potvrdily plánovanou účast, s e-mailem a školou nebo organizací. Nejde o evidenci skutečného sledování vysílání.</p>
      ...
      {data && <>
        ...
        <div className="overflow-x-auto rounded-2xl border bg-white">
          <table className="w-full text-left"><caption className="sr-only">Seznam potvrzených účastí</caption>
            <thead className="bg-slate-50"><tr>{["Škola / organizace", "Jméno účastníka", "E-mail", "Datum přihlášení"].map((v) => <th scope="col" className="p-4" key={v}>{v}</th>)}</tr></thead>
          ...
          {!rows.length && <p className="p-5 text-slate-600">{data.attendees.length ? "Hledání neodpovídá žádná přihláška." : "Zatím se nepřihlásil žádný účastník."}</p>}
        </div>
      </>}
    </main>
  </RequireAuth>;
