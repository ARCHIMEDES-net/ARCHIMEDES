...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!UUID_PATTERN.test(sourceAttemptId) || resolutionReason.length < 20 || resolutionReason.length > 1000) {
      return res.status(400).json({ error: "Neplatné ID nebo nedostatečný důvod uzavření." });
    }
    if (req.body?.confirmation !== "RESOLVE_WITHOUT_RESEND") {
      return res.status(400).json({ error: "Chybí výslovné potvrzení uzavření bez odeslání." });
    }
    const allowed = await consumeAuthenticatedRateLimit({
      supabaseAdmin,
      req,
      route: "admin-resolve-profile-reminder",
    ...
    const { data, error } = await supabaseAdmin.rpc(
      "resolve_profile_reminder_without_resend",
      {
    ...
    if (data !== true) {
      return res.status(409).json({ error: "Případ nelze uzavřít nebo už byl vyřešen." });
    }
    return res.status(200).json({ ok: true, emailSent: false });
  } catch (error) {
    console.error("resolve-profile-reminder error:", error);
    return res.status(500).json({ error: "Případ se nepodařilo bezpečně uzavřít." });
  }
}
