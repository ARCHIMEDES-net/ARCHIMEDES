...
const OIDC_AUDIENCE = "archimedes-onboarding-e2e-pr198";
...
function workflowRefMatches(workflowRef) {
  return (
    workflowRef === `${EXPECTED_WORKFLOW_PATH}refs/pull/198/merge` ||
    workflowRef ===
      `${EXPECTED_WORKFLOW_PATH}refs/heads/agent/smtp-registration-hardening`
  );
}
...
async function verifyGitHubOidcToken(token) {
  ...
  if (
    ...
    payload.ref !== "refs/pull/198/merge" ||
    payload.head_ref !== "agent/smtp-registration-hardening" ||
    payload.base_ref !== "main" ||
    !subjectMatches(payload) ||
    !workflowRefMatches(payload.workflow_ref) ||
    payload.workflow !== "Controlled onboarding E2E for PR 198 via OIDC" ||
