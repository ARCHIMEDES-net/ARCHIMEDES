...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: orderRows, error: dbError } = await supabase
      .from("orders_start")
      .insert([
        {
    ...
    try {
      ...
      if (existingOrganizationError) {
        throw new Error(
          `Nepodařilo se ověřit existenci organizace: ${existingOrganizationError.message}`
        );
      }

      if (existingOrganization) {
        organizationId = existingOrganization.id;
        joinCode = existingOrganization.join_code;
      } else {
        ...
        if (createOrganizationError || !createdOrganization?.id) {
          throw new Error(
            `Nepodařilo se vytvořit organizaci: ${createOrganizationError?.message || "neznámá chyba"}`
          );
        }

        organizationId = createdOrganization.id;
        joinCode = createdOrganization.join_code;
      }

      const { data: invitedUser, error: inviteError } =
        await supabase.auth.admin.inviteUserByEmail(adminEmailRaw, {
          redirectTo: REDIRECT_TO,
          data: {
            full_name: contactNameRaw,
          },
        });

      if (inviteError) {
        throw new Error(`Nepodařilo se pozvat administrátora: ${inviteError.message}`);
      }
      ...
      if (membershipLookupError) {
        throw new Error(
          `Nepodařilo se ověřit existující členství administrátora: ${membershipLookupError.message}`
        );
      }
      ...
      if (profileError) {
        throw new Error(`Nepodařilo se uložit profil administrátora: ${profileError.message}`);
      }
      ...
      if (membershipError) {
        throw new Error(
          `Nepodařilo se přiřadit administrátora do organizace: ${membershipError.message}`
        );
      }
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p><strong>Join code:</strong> ${safeJoinCode}</p>
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>
          Administrátorský přístup do portálu připravujeme pro e-mail:
