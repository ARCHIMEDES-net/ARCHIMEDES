...
const SINGLE_LETTER_WORD = /(^|[\s([{„\"'])\b([AaIiKkOoSsUuVvZz])[ \t\r\n]+(?=\S)/g;

export function addCzechNonBreakingSpaces(value) {
  let result = value;
  let previous;

  // Repeat so sequences such as “a i další” keep both one-letter words
  // attached to what follows.
  do {
    previous = result;
    result = result.replace(SINGLE_LETTER_WORD, "$1$2\u00a0");
...
export function applyCzechNonBreakingSpaces(root) {
  ...
  const processTextNode = (node) => {
    ...
    const nextValue = addCzechNonBreakingSpaces(node.nodeValue);
