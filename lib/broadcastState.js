import { hasArchiveRecording } from "./archiveRecording";
...
export function getJoinButtonState(event, now = new Date()) {
  ...
  const session = normalizeBroadcastSession(event?.broadcast_sessions || event?.broadcast_session);
  const dynamicJoin = Boolean(session?.external_meeting_id);
  const href = getStreamUrl(event);
  const hasJoinTarget = Boolean(href || dynamicJoin);
  const status = getBroadcastStatus(event);
  const { start, openFrom, openUntil } = getJoinWindow(event);
  const archiveReady = hasArchiveRecording(event);
  ...
  if (!start || !openFrom || !openUntil) {
    if (status === "done") {
      return archiveReady
        ...
        : {
            state: "recording",
            label: "Záznam připravujeme",
            href: "",
            start,
            reason: "recording-missing-no-date",
          };
    }
  ...
  if (status === "done" || now > openUntil) {
    return archiveReady
      ? {
          ...
          openFrom,
          openUntil,
          reason: "after-window-with-recording",
        }
      : {
          state: "recording",
          label: "Záznam připravujeme",
          ...
          openFrom,
          openUntil,
          reason: "after-window-without-recording",
        };
  }
  ...
  if (now < openFrom) {
    return {
      state: "waiting",
      label: openFrom.toLocaleTimeString("cs-CZ", {
        hour: "2-digit",
        minute: "2-digit",
      }),
      href: "",
