...
export async function sendCustomerOnboardingEmail({
  ...
  const accessRole = memberRole === "member" ? "učitele" : "lokálního správce";
  ...
  const nextStepUrl = isMunicipality
    ...
    : organizationType === "school" && memberRole !== "member"
...
byl vám připraven přístup ${accessRole} platformy ARCHIMEDES Live pro organizaci ${organizationName}.
...
<html lang="cs">
  ...
  <body>
    ...
    <p>byl vám připraven přístup ${accessRole} platformy ARCHIMEDES Live pro organizaci ${escapeHtml(
      organizationName
    )}.</p>
    <ul>
  ...
  return sendRegistrationEmail({
    to: email,
    subject: `ARCHIMEDES Live – přístup ${memberRole === "member" ? "učitele" : "správce"} pro ${organizationName}`,
    text,
    html,
    idempotencyKey,
  });
...
export function customerOnboardingAuditCopyMessage({
  ...
  const roleNoun = memberRole === "member" ? "učitele" : "správce";
  const roleLabel = memberRole === "member" ? "Učitel" : "Správce";
  const validUntilText = licenseValidUntil
    ? new Date(licenseValidUntil).toLocaleDateString("cs-CZ")
    : "bez uvedeného konce";
  const subject = `ARCHIMEDES Live – kopie registrace ${roleNoun} | ${organizationName}`;
  const text = `Bezpečná auditní kopie registrace ${roleNoun}.
...
<html lang="cs">
  ...
  <body>
    <p><strong>Bezpečná auditní kopie registrace ${roleNoun}.</strong></p>
    <ul>
      ...
      <li><strong>${roleLabel}:</strong> ${escapeHtml(fullName)}</li>
