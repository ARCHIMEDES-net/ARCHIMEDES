...
function createCustomerOnboardingTransport() {
...
export async function verifyCustomerOnboardingEmailTransport() {
  const transporter = createCustomerOnboardingTransport();
...
<html lang="cs">
  ...
  return sendSmtpMessage(transporter, {
...
export async function sendCustomerOnboardingAuditCopy(values) {
  const transporter = createCustomerOnboardingTransport();
...
<body style="font-family:Segoe UI,Arial,sans-serif;color:#0f172a;line-height:1.65">
  ...
  await sendSmtpMessage(transporter, {
    from: process.env.MAIL_FROM,
    to: email,
    subject,
    text,
