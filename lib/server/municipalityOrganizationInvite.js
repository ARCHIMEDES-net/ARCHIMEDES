...
export async function inspectMunicipalityInvite({
  supabaseAdmin,
  rawToken,
  organizationType,
}) {
  ...
  // Pozvánky vznikají z 32 náhodných bajtů v base64url formátu. Neplatné
  // nebo nepřiměřeně dlouhé vstupy odmítneme ještě před hashováním a dotazem.
  ...
  const { data: municipality, error: municipalityError } = await supabaseAdmin
    .from("organizations")
    .select("id, name, status, license_status, license_plan, license_valid_until")
...
export async function resolveMunicipalityInvite({
  ...
  const context = await inspectMunicipalityInvite({
    supabaseAdmin,
    rawToken,
    organizationType,
  });
  const invitedEmail = String(context.invite.invited_email || "")
    .trim()
    .toLowerCase();
