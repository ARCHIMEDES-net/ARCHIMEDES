...
export async function resolveOrganizationRegistrant({
  ...
  if (!userId) {
    ...
    if (linkError || !setupUrl) {
      await cleanupNewRegistrant(
        supabaseAdmin,
        { userId, isNewAccount: true },
        { phase: "generate-setup-link" }
      );
...
export async function cleanupNewRegistrant(
  supabaseAdmin,
  registrant,
  context = {}
) {
  if (!registrant?.userId || !registrant.isNewAccount) {
    return { attempted: false, succeeded: true };
  }

  try {
    const deletion = await supabaseAdmin.auth.admin.deleteUser(registrant.userId);
    if (deletion?.error) {
      throw deletion.error;
    }

    console.info("organization registrant cleanup succeeded", {
      userId: registrant.userId,
      ...context,
    });
    return { attempted: true, succeeded: true };
  } catch (error) {
    console.error("organization registrant cleanup failed", {
      userId: registrant.userId,
