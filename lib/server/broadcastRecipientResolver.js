...
export async function resolveWebMeetingParticipants(
  ...
  let manualProfiles = [];
  // Keep REST query URLs bounded for large invitation lists.
  for (let offset = 0; offset < manual.emails.length; offset += 50) {
    const { data, error } = await supabaseAdmin
      .from("profiles")
      .select("id, email, full_name")
      .in("email", manual.emails.slice(offset, offset + 50));
    if (error) throw error;
    manualProfiles.push(...(data || []));
  }
