...
function hasActiveLicense(organization, now = new Date()) {
  ...
  if (organization.license_plan === "competition_prize_12m" &&
      (!organization.license_started_at || new Date(organization.license_started_at) > now || !organization.license_valid_until)) return false;
...
export async function requireBroadcastViewer(req, supabaseAdmin) {
  ...
  const { data: organization, error: organizationError } = await supabaseAdmin
    .from("organizations")
    .select("id, org_type, status, license_status, license_plan, license_started_at, license_valid_until, parent_organization_id")
  ...
  if (hasActiveLicense(organization) && (!organization.parent_organization_id || organization.license_plan)) return identity;
  ...
  if (organization.parent_organization_id) {
    ...
    if (parent?.license_plan !== "competition_prize_12m" && hasActiveLicense(organization)) return identity;
    if (
      parent && parent.license_plan !== "competition_prize_12m" &&
...
export function webMeetingParticipantNumber(userId) {
  const hex = String(userId || "").replace(/-/g, "");
  if (!/^[0-9a-f]{32}$/i.test(hex)) {
    throw new BroadcastAccessError("Uživatel nemá platný technický identifikátor.", 409);
  }

  // WebMeeting vyžaduje v poli `number` celé číslo, zatímco Supabase používá UUID.
  // Celé UUID deterministicky zmenšíme do kladného rozsahu signed 32-bit integeru.
  return Number((BigInt(`0x${hex}`) % 2147483646n) + 1n);
}
...
export function webMeetingParticipant(identity) {
  ...
  return {
    number: webMeetingParticipantNumber(identity.user.id),
    firstname,
    surname,
