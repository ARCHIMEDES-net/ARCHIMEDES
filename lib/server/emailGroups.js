// Přechodové mapování zachovává původní skupiny z user_interests a
// současně je slučuje s novým číselníkem notification_preferences.
...
export async function getEmailGroups(supabaseAdmin) {
  const [categoriesResult, preferencesResult, legacyResult] = await Promise.all([
    ...
    supabaseAdmin
      ...
      .select("profile_id, activity_code")
      .eq("enabled", true),
