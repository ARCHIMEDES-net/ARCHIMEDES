...
export function buildNotificationCandidates({
  ...
  for (const session of sessions) {
    ...
    for (const groupCode of unique(session.recipient_group_codes || [])) {
    ...
    for (const profileId of groupRecipients) {
      ...
      appendCandidate({
        ...
        // Nové pořady patří do centra novinek. Jednotlivé e-maily/push by při
        // publikaci více pořadů vytvořily dávku zpráv; externí digest přijde až
        // jako samostatně řízená funkce.
...
export async function prepareNotificationQueue(supabaseAdmin, { now = new Date(), preview = false } = {}) {
  ...
  const eventIds = unique(sessions.map((session) => session.event_id));
  const groupCodes = unique(sessions.flatMap((session) => session.recipient_group_codes || []));
