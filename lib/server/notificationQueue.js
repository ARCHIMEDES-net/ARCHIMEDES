    ...
    for (const profileId of subscriptionsByEvent.get(eventId) || []) {
      ...
      for (const minutesBefore of offsets) {
        ...
        appendCandidate({
          ...
          // E-mailové upomínky vlastní trvalá broadcast mail fronta.
          // Zde zůstávají interní oznámení a případný denní push.
          deliveryPolicy: minutesBefore === HALF_HOUR_MINUTES
            ? "in_app_only"
            : ["archimedes_all", "in_app_and_push"].includes(deliveryPolicy)
              ? "in_app_and_push" : "in_app_only",
        });
      }
    }
  }
...
export async function prepareNotificationQueue(supabaseAdmin, { now = new Date(), preview = false } = {}) {
  ...
  const availableGroupCodes = availableGroups.map((group) => group.code).filter(Boolean);
  const standaloneEvents = publishedEvents
    .filter((event) => !sessionEventIds.has(event.id))
    .map((event) => ({
      ...event,
      recipient_group_codes: suggestRecipientGroups(event, availableGroupCodes),
    }));
