...
function responseSignature(headers) {
  ...
  const match = authorization.match(/^SaltedChecksum:\s*(\S+)$/i);
...
export async function callWebMeeting(action, parameters = {}) {
  ...
  if (!receivedSignature || receivedSignature.length !== expectedSignature.length) {
    throw new WebMeetingApiError("Odpověď WebMeetingu nemá platný bezpečnostní podpis.", {
      status: 502,
      action,
    });
  }
...
export const webMeeting = {
  getMeetings(meetingId = null) {
    return callWebMeeting("getMeetings", meetingId == null ? {} : { meetingId });
  },
  createMeeting({ name, startsAt, speakerName = "", description = "", type = 11 }) {
  ...
  getPresence(meetingId, participantId = null) {
    return callWebMeeting(
      "getPresence",
      participantId == null ? { meetingId } : { meetingId, participantId }
    );
  },
};
