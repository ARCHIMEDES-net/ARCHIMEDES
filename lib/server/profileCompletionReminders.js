...
export async function processProfileCompletionReminders(
  ...
  try {
    await verifySmtpTransport(mailer);
  } catch (smtpError) {
    logSafeSmtpFailure(
      "profile completion reminders SMTP preflight failed",
      smtpError,
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      logSafeSmtpFailure(
        "profile completion reminder delivery unconfirmed",
        smtpError,
        { attemptId, profileId: candidate.profile.id, reminderStep: candidate.step }
      );
      await supabaseAdmin
        .from("profile_completion_reminder_attempts")
    ...
    try {
      ...
      logSafeSmtpFailure(
        "profile completion reminder audit copy failed",
        smtpError,
        { attemptId, profileId: candidate.profile.id, reminderStep: candidate.step }
      );
      result.copyFailed += 1;
