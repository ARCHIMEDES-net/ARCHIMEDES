...
import { sendRegistrationEmail } from "./registrationEmailProvider";
...
export async function processProfileCompletionReminders(
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      await supabaseAdmin
        .from("profile_completion_reminder_attempts")
        .update({ status: "delivery_unknown", error_code: "registration_email_delivery_unknown", updated_at: new Date().toISOString() })
        .eq("id", attemptId)
        .eq("status", "sending");
