...
function validationErrorCode(message) {
  ...
  if (normalized.includes("only send testing emails to your own email address")) {
    return "REGISTRATION_EMAIL_PROVIDER_DOMAIN_NOT_ENABLED";
  }
  if (normalized.includes("domain is not verified") || normalized.includes("domain is not verified.")) {
    return "REGISTRATION_EMAIL_PROVIDER_DOMAIN_NOT_VERIFIED";
  }
  if (normalized.includes("domain has been registered already")) {
    return "REGISTRATION_EMAIL_PROVIDER_DOMAIN_REGISTERED_ELSEWHERE";
  }
  return "REGISTRATION_EMAIL_PROVIDER_VALIDATION_ERROR";
}

function rejectedProviderCode(providerErrorName, providerMessage) {
  ...
  if (providerErrorName === "validation_error") {
    return validationErrorCode(providerMessage);
  }
    ...
    if (!response.ok || !payload?.id) {
      ...
      throw new RegistrationEmailProviderError(
        ...
        response.ok
          ...
          : rejectedProviderCode(providerErrorName, payload?.message),
        {
