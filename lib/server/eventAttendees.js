...
export async function loadEventAttendees(db, eventId) {
  ...
  for (let offset = 0; ; offset += 200) {
    ...
    const [organizations, profiles] = await Promise.all([
      db.from("organizations").select("id, name").in("id", [...new Set(rows.map((r) => r.organization_id).filter(Boolean))]),
    ...
    result.push(...rows.map((r) => ({
      id: r.id,
      organization: r.organization_id ? (orgs.get(r.organization_id)?.name || "Organizace již není dostupná") : "Správce platformy – osobní účast",
...
${row([title])}${row(["Škola / organizace", "Jméno účastníka", "E-mail", "Datum přihlášení (Praha)"])}
