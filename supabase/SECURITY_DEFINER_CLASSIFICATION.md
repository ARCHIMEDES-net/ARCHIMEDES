...
| `has_access_program(uuid)` | RLS helper chráněných programů | Zachovat; vrací přístup jen aktivnímu členovi aktivní organizace přiřazené k programu nebo platformovému správci. `PUBLIC` a `anon` nemají `EXECUTE`, funkce nemění data a používá prázdný `search_path`. |
| `can_administer_access_program(uuid)` | RLS helper správy programu | Zachovat; vyžaduje aktivní roli `organization_admin` v aktivní vlastnické organizaci programu nebo platformového správce. Slouží pro metadata a soubory v privátním bucketu, `PUBLIC` a `anon` nemají `EXECUTE`. |
| `get_access_program_organizations(uuid)` | omezené administrační RPC | Zachovat; vrací pouze základní organizační a kontaktní údaje a pouze uživateli, který projde `can_administer_access_program(...)`. Nevystavuje registrační kódy ani licenční data. |
